...
files = list.files(GLOBAL_SIZE_TEST_WINDOWS_DIR, pattern= paste0(".*", file_tag, ".*") )
print(paste0(Sys.time(), "    found ", length(files), " files containing the tag: ", file_tag))

i = 1
for (f in files) {
	print( paste0(Sys.time(), "    ", i, "/",  length(files)) )
	panel_windows = scan(paste0(GLOBAL_SIZE_TEST_WINDOWS_DIR, f), what=character(), quiet=TRUE)
	...
	s = sub("trunc_panel_windows_", "", f)
	s = sub(".txt", "", s)

	outfile = paste0(GLOBAL_SIZE_TEST_SBS_DIR, "trunc_sbs_df_", s, ".tsv")
