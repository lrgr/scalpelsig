# this script will train ScalpelSig on all samples in the given cohort.
# no need to run initialize_10k_panel_script.R with this script, it handles the initialization itself.
...
option_list = list(
	...
	make_option(c("-o", "--objectivefn"), type="numeric", default=2, 
			help="controls which objective function the panel will use, can be 1, 2, or 3", metavar="character"),
	make_option(c("-w", "--windowsinpanel"), type="numeric", default=250,
...
print(paste0(Sys.time(), "    Running full_train_panel_script.R - recieved the following inputs:"))
print(paste0("sig_num: ", sig_num))
print(paste0("obj_fn_num: ", obj_fn_num))
print(paste0("windows_in_panel: ", windows_in_panel))
print(paste0("file_tag: ", file_tag))
...
train_set = samp_names # train on ALL samples in the cohort
...
window_file_name = paste0(GLOBAL_SCRIPT_PANEL_WINDOWS_DIR, "ft_panel_windows_",  file_tag, "_sig", sig_num, "_obj", obj_fn_num, "_nwin", windows_in_panel, "_", timestamp_tag, ".txt")
