...
# sbatch SLURM_10k_panel_script.sh <signature number> <file_tag> 

# Lines that begin with #SBATCH specify commands to be used by SLURM for scheduling

#SBATCH --job-name=compute_10kb_panel                                   # sets the job name
#SBATCH --output out/SCRIPT_OUTS/SLURM_LOGS/10kb_panel_log_jobid%j.txt                             # indicates a file to redirect STDOUT to; %j is the jobid
#SBATCH --error out/SCRIPT_OUTS/SLURM_LOGS/10kb_panel_error_jobid%j.txt                              # indicates a file to redirect STDERR to; %j is the jobid
