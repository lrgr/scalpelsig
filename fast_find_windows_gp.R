# THIS SCRIPT MODIFIES run_10k_panel_script.R
# it is very similar, it just changes some bookkeeping
# the idea is that we will compute the objective function on the training data exactly as before,
# but in the next step in the workflow we will incorporate some windows from each signature's top n
# into the general panel.
...
print(paste0(Sys.time(), "    STARTED SCRIPT"))
...
# the iteration number is not mentioned here because the SLURM script treats it as part of the file tag.
test_train_file = paste0(GLOBAL_GP_TEST_TRAIN_DIR, "gp_test_train_", file_tag, ".rds")
