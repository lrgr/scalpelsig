...
# sbatch SLURM_activity_thresh_script.sh <signature number> <file_tag> <activation_threshold>

# Lines that begin with #SBATCH specify commands to be used by SLURM for scheduling

#SBATCH --job-name=compute_10kb_panel                                   # sets the job name
#SBATCH --output out/SCRIPT_OUTS/SLURM_LOGS/act_thresh_panel_log_jobid%j.txt                             # indicates a file to redirect STDOUT to; %j is the jobid
#SBATCH --error out/SCRIPT_OUTS/SLURM_LOGS/act_thresh_panel_error_jobid%j.txt                              # indicates a file to redirect STDERR to; %j is the jobid
...
for i in {1..5}; do
    for obj in {1..3}; do
        srun -N 1 -n 1 --mem=35gb --exclusive Rscript run_10k_panel_script.R -s ${1} -t ${2}_it${i} -o ${obj} -w 250 -a ${3} &
    done
done
